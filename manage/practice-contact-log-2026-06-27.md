...
|---|---|---|---|---|---|
| 1 | `HuangYuChuh/ComfyUI_Skills_OpenClaw` | GitHub code search：`ComfyUI_Skills_OpenClaw prompt generated image`；GitHub issue search：`"ComfyUI_Skills_OpenClaw" "generated image"`；查看 repo issues；访问 issue #129 的 Socialistic runner。 | code / issue search 未返回完整输出链路；issue #129 提供 Socialistic 在线试跑入口；页面可访问，OG 描述为 `Upload a workflow, prompt, image, or mask，直接得到结果。`，能证明 runner 存在，但未公开单次 prompt、workflow history、generated image 或复盘。 | 继续追证，不入库。 | 联系作者或 issue #129 作者，索取一条公开示例：prompt、workflow/schema、history/log、生成图、结论。 |
